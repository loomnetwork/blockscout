defmodule Explorer.Token.InstanceMetadataRetriever do
  ...
  def fetch_json(%{"tokenURI" => {:ok, ["data:application/json," <> json]}}) do
    ...
    e ->
      Logger.warn(["Unknown metadata format #{inspect(json)}. error #{inspect(e)}"],
  ...
  def fetch_json(result) do
    Logger.warn(["Unknown metadata format #{inspect(result)}."], fetcher: :token_instances)
  ...
  defp fetch_metadata(token_uri) do
    ...
    e ->
      Logger.warn(["Could not send request to token uri #{inspect(token_uri)}. error #{inspect(e)}"],
        fetcher: :token_instances
      )
